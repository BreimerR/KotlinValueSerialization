...
open class SerializerIrGenerator(
    ...
    override fun generateSave(function: FunctionDescriptor) = irClass.contributeFunction(function) { saveFunc ->
        ...
        val beginFunc = encoderClass.referenceFunctionSymbol(CallingConventions.begin) { it->it.valueParameters.size == 1 }

        val call:IrFunctionAccessExpression = irCall(beginFunc, type = kOutputClass.defaultType.toIrType()).mapValueParametersIndexed { _, _ ->
    ...
    override fun generateLoad(function: FunctionDescriptor) = irClass.contributeFunction(function) { loadFunc ->
        ...
        val beginFunc = decoderClass.referenceFunctionSymbol(CallingConventions.begin) { it->it.valueParameters.size == 1 }
        val call = irInvoke(
                ...
                val body = irBlock {
                    val decodeFuncToCall = formEncodeDecodePropertyCall(localInput.get(), loadFunc.dispatchReceiverParameter!!, property, {innerSerial, sti ->
                        inputClass.referenceFunctionSymbol(
                            "${CallingConventions.decode}${sti.elementMethodPrefix}Serializable${CallingConventions.elementPostfix}", {it-> it.valueParameters.size == 4}
                        ) to listOf(
