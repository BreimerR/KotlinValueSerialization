...
package libetal.kotlinx.serialization.compiler.diagnostic
...
import libetal.kotlinx.serialization.compiler.resolve.serialTransient
import libetal.kotlinx.serialization.compiler.resolve.serializableWith
...
import libetal.kotlinx.serialization.compiler.backend.common.bodyPropertiesDescriptorsMap
import libetal.kotlinx.serialization.compiler.backend.common.findTypeSerializerOrContextUnchecked
import libetal.kotlinx.serialization.compiler.backend.common.primaryConstructorPropertiesDescriptorsMap
import libetal.kotlinx.serialization.compiler.resolve.SerializationAnnotations.serializableAnnotationFqName
    ...
    private fun canBeSerializedInternally(descriptor: ClassDescriptor, declaration: KtDeclaration, trace: BindingTrace): Boolean {
        ...
        if (!descriptor.isSerializableEnum()) { // enums are inherited from java.lang.Enum and can't be inherited from other classes
            ...
            if (!superClass.isInternalSerializable && superClass.constructors.singleOrNull { it.valueParameters.size == 0 } == null) {
                trace.reportOnSerializableAnnotation(descriptor,
